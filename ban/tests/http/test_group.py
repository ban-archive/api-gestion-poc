...
def test_get_group_housenumbers(get, url):
    ...
    resp = get(url('housenumber-collection', group=street.id))
    assert resp.status_code == 200
...
def test_create_group(client, url):
    ...
    uri = url('group-resource', identifier=resp.json['id'])
...
def test_get_group_versions(get, url):
    ...
    street.save()
    uri = url('group_version_collection', identifier=street.id)
    resp = get(uri)
    assert resp.status_code == 200
...
def test_get_group_version(get, url):
    ...
    street.name = "Rue de la Guerre"
    street.save()
    uri = url('group-version-by-sequential', identifier=street.id, ref=1)
    resp = get(uri)
    assert resp.status_code == 200
    assert resp.json['data']['name'] == 'Rue de la Paix'
    assert resp.json['data']['version'] == 1
    uri = url('group-version-by-sequential', identifier=street.id, ref=2)
    resp = get(uri)
    assert resp.status_code == 200
